...
for city in city_df.collect():
    humid_df = spark.read.csv(f'{file_path}/temp/{city["City"]}_humid/*.csv', header=True)
    temp_df = spark.read.csv(f'{file_path}/temp/{city["City"]}_temp/*.csv', header=True)
    wind_df = spark.read.csv(f'{file_path}/temp/{city["City"]}_wind/*.csv', header=True)
    ...
    final_df.coalesce(1).write.csv(f'{file_path}/Dataset/city/{city["City"]}', header=True, mode="ignore")
